...
class Model{
    ...
    func createCell(indexPath:IndexPath, bucketItems:[bucketItem], collectionView:UICollectionView) -> uploadedCell{
        ...
        if !bucketItems[indexPath.row].folder{
            cell.folder = false
            cell.thumbnail.setImage(url: bucketItems[indexPath.row].thumnailUrl!)
    ...
    func setHeaderView(currentPage:Int,totalImages:Int,delegate:headerImageDelegate, controller:LightboxController) -> UIView{
        ...
        view.text_label.text = "\(currentPage + 1) of \(totalImages)"
    ...
    func getUploadedImages(documentRef:CollectionReference, onComplete:@escaping ([String],[bucketItem],[String]) -> Void){
        var imageID = [String]()
        var bucketItems = [bucketItem]()
        var folderNames = [String]()
        documentRef.order(by: "Date", descending: true)
